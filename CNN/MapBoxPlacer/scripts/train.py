...
def train(args):
    ...
    Xt,yt=X[idx[:-nv]],y[idx[:-nv]]; Xv_,yv_=X[idx[-nv:]],y[idx[-nv:]]
    print(f"训练:{len(Xt):,} 验证:{len(Xv_):,}")
    Xtt=torch.from_numpy(Xt).float(); ytt=torch.from_numpy(yt).float()
    Xvt=torch.from_numpy(Xv_).float(); yvt=torch.from_numpy(yv_).float()
    tr=DataLoader(TensorDataset(Xtt,ytt),BATCH,shuffle=True,pin_memory=True)
    va=DataLoader(TensorDataset(Xvt,yvt),BATCH*2,pin_memory=True)
    ...
    for ep in range(sp,EPOCHS+1):
        ...
        with torch.no_grad():
            for bx,by in va:
                bx,by=bx.to(dev),by.to(dev); mk=bx[:,1:2]
                nv_=(bx[:,0,0,0]*MAX_N).round().long(); lo=mask_loss(model(bx),by,mk)
                vl+=lo.item()*bx.size(0); ap.append(torch.sigmoid(model(bx))); at.append(by); am.append(mk); an.append(nv_)
