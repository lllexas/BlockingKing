"""
GoalPlacer 数据解析: 箱图 → 目标热力图
X: [2,20,20] (箱二值图 + valid_mask), y: [1,20,20] (目标二值图)
...
def make_sample(window):
    ...
    y[0]=((window==3)|(window==6)).astype(np.float32)   # 目标
    return X,y
...
def main():
    ...
    print(f"GoalPlacer 数据解析, 窗口 {MIN_SIZE}~{MAX_SIZE}")
    Xl,yl=[],[]; raw=0
    for fn in files:
